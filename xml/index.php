...
<body>
    ...
    <div class="container">
        <div class="row g-3">
            ...
            <!-- times -->
            <div class="col-6 col-md-3">
               <a href="/xml/times.php">
                    <div class="painel-card">
                        <i class="fa-solid fa-futbol text-info"></i>
                        <h6><p id="contadorTimes">0</p>Escudos Times</h6>
                    </div>
                </a>
            </div>
            ...
            <!-- bandeiras -->
            <div class="col-6 col-md-3">
               <a href="/xml/bandeiras.php">
                    <div class="painel-card">
                        <i class="fa-solid fa-flag text-info"></i>
                        <h6><p id="contadorFlags">0</p>Bandeiras</h6>
                    </div>
                </a>
            </div>
  ...
  <script>
async function carregarImagens(config) {

    const apiURL = `https://api.github.com/repos/${config.owner}/${config.repo}/git/trees/${config.branch}?recursive=1`;
    const response = await fetch(apiURL);
    const data = await response.json();

    const imagens = data.tree.filter(item =>
        item.type === "blob" &&
        /\.(png|jpg|jpeg|svg|gif|webp)$/i.test(item.path)
    );

    // contador
    document.getElementById(config.contador).textContent = imagens.length;

    // monta galeria
    const galeria = document.getElementById(config.galeria);
    galeria.innerHTML = imagens.map(img => `
        <div class="col-6 col-md-3">
            <div class="card shadow-sm p-1">
                <img src="${config.baseURL + img.path}" class="img-fluid">
                <small>${img.path}</small>
            </div>
        </div>
    `).join("");

    // salvar lista para filtro
    config.lista = imagens;

    // filtro individual
    document.getElementById(config.busca).addEventListener("input", e => {
        const termo = e.target.value.toLowerCase();

        const filtradas = config.lista.filter(x =>
            x.path.toLowerCase().includes(termo)
        );
        ...
        document.getElementById(config.contador).textContent = filtradas.length;
    });
}
...
const pastaTimes = {
    ...
    repo: "escudos",
    branch: "main",
    galeria: "galeriaTimes",
    contador: "contadorTimes",
    busca: "buscaTimes"
};
pastaTimes.baseURL = `https://cdn.jsdelivr.net/gh/${pastaTimes.owner}/${pastaTimes.repo}@${pastaTimes.branch}/`;

const pastaFlags = {
    owner: "cybercoari-1",
    repo: "flags",
    branch: "main",
    galeria: "galeriaFlags",
    contador: "contadorFlags",
    busca: "buscaFlags"
};
pastaFlags.baseURL = `https://cdn.jsdelivr.net/gh/${pastaFlags.owner}/${pastaFlags.repo}@${pastaFlags.branch}/`;

/* INICIAR */
document.addEventListener("DOMContentLoaded", () => {
    carregarImagens(pastaTimes);
    carregarImagens(pastaFlags);
});
</script>
