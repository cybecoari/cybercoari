...
.controls { display:flex; justify-content:center; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:5px; }
button {
...
nextBtn.onclick = () => {
  ...
  if (isShuffle) {
    playMusicTrack(Math.floor(Math.random() * musicList.length));
  } else {
    playMusicTrack((currentTrack + 1) % musicList.length);
  }
};
...
prevBtn.onclick = () => {
  ...
  if (isShuffle) {
    ...
    playMusicTrack((currentTrack - 1 + musicList.length) % musicList.length);
  }
};
...
shuffleBtn.onclick = () => {
  ...
  statusEl.textContent = isShuffle ? '🔀 Modo aleatório ligado' : 'Modo aleatório desligado';
  statusEl.className = 'status';
};
